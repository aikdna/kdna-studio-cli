...
async function verify(root) {
  ...
  const rows = await runWithConcurrency(
    ...
    async (entry) => {
      ...
      if (commitIsUsable && pathIsUsable) {
        try {
          row.lastAppearance = lastAppearanceCommit(root, row.originalPath, graph);
          ...
          const window = retirementWindow(root, row.originalPath, entry.file, graph);
          if (window !== null) {
            ...
            if (afterMove !== null) {
              ...
              for (const parent of window.parents) {
                const beforeMove = blobText(root, parent.commit, row.originalPath);
                if (beforeMove === afterMove) continue;
                row.retirementRewroteContent = true;
                row.retirementRewriteDiff.push(...lineDiff(
                  beforeMove,
                  afterMove,
                  `${window.first.slice(0, 12)}${parent.suffix}:${row.originalPath}`,
